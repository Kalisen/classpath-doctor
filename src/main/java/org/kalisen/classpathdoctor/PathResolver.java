...
public class PathResolver {
	...
	public PathEntry resolve(String path) {
		...
		if (path == null) {
			result = new InvalidPathEntry(path, ResourceBundle.getBundle(
					"UsersMessages").getString("null.is.not.a.valid.path"));
		} else if ("".equals(path)) {
			...
			try {
				...
				if (f.isDirectory()) {
					...
					result = new InvalidPathEntry(expandedPath, ResourceBundle
